...
export default class ShoppingCart {
  ...
  transferLocalToState(session) {
    ...
    if (!session.state[session.state.sessionOwner].shoppingCart || !session.state[session.state.sessionOwner].shoppingCart.length) {
      this.seedStateCart(session);
      return;
    }
  ...
  addProduct(session, products, addThis) {
    ...
    // these two statements handle the case where a user, either anonymous or named
    // does not have a shopping cart
    let shoppingCart = session.state[session.state.sessionOwner].shoppingCart || [];
    session.state[session.state.sessionOwner].shoppingCart = shoppingCart;
  ...
  // capability = req.app.locals.capability
  // session = req.session
  // cartA cartA gets merged into cartB
  mergeTwoCarts(capability, session, products, cartA) {
    let payload = {};
    for (const item of cartA) {
      payload = {
        ...
        rows: this.decodeRowsId(item.productId, item.rowsId),
      }
      this.shoppingCartControlCenter(capability, session, products, payload);
    }
  }

  decodeRowsId(productId, rowsId) {
    console.log(Chalk.yellow.bold("<<<------ STARTING THE ROWS ID DECODING PROCESS ------->>>"));
    const ids = rowsId.substring(3);
    let rows = {};
    switch (productId) {
    case "100":
      rows = {
    ...
    case "300":
      break;
    default:
      console.log("wrong productId provided");
      break;
    }
    console.log(Chalk.yellow.bold("<<<-------- ROWS ID DECODING PROCCESS FINISHED ------->>>"));
    return rows;
  }
}
