...
class Capability {
  ...
  // session = req.session
  // sessionOwner = "string";
  seedState(session, sessionOwner = "anonymous") {
    if (!session.state) {
      console.log(Chalk.blue.bold("CREATION OF STATE IN REQ.SESSION"));
      session.state = {};
    }
...
class User {
  ...
  login(res, session, products) {
    switch (session.state.login.message) {
    case "success": // only on a successfull login is the postlogin function invoked
      res.locals.capability.Communicate.loginResult(res.locals.capability, { message: "success" }, session.state);
      res.locals.capability.User.postLogin(res.locals.capability, session, products);
      break;
    case "dbError":
  ...
  postLogin(capability, session) {
    ...
    //capability.ShoppingCart.mergeTwoCarts(capability, session, session.state.stateCart.shoppingCart);
  ...
  makeUserName(user) {
    ...
    if (user.username.length > 7) user.username = user.username.substr(0, 7);
  }

  // capability = req.app.locals.capability
  // session = req.session
  async saveShoppingCart(capability, session) {
    // get by user._id
    // get the new shoppingCart
    let newCart = session.state[session.state.sessionOwner].shoppingCart;
    if (Array.isArray(newCart)) {
      // turn the array into a string
      newCart = capability.ShoppingCart.toString(newCart);
    } else {
      console.log("the new cart is not an array");
    }

    // now it is ready to have the database be updated with the new object
    try {
      let a = await capability.Database.updateDocumentDb("mongo", "email", session.state.user.email, {shoppingCart: newCart}, "costumer");
      console.log(a);
    } catch (err) {
      console.log(err);
    }
  ...
  // capability = req.app.locals.capability
  // session = req.session
  async logout(capability, session) {
    console.log(Chalk.yellow.bold("INITIATING LOGOUT PROCCESS"));
    try {
      await this.saveShoppingCart(capability, session);
    } catch (err) {
      console.log(err);
    }
...
class Anonymous {
  ...
  hasShoppingCart(session) {
    if (!session.state.anonymous.shoppingCart || !session.state.anonymous.shoppingCart.length) {
      return false;
    } else {
      console.log(Chalk.red.bold("ANONYMOUS USER HAS A SHOPPING CART"));
      return true;
    }
  }
}
...
class FlashMessage {
  ...
  transferShoppingCart() {
    // this flashMessage id = flash200
    const standardContent = '<aside class="popup-container flash200" >' +
          `<p id="popup-message-flash200">Previous user left a shopping Cart!</p>` +
          `<p>Should we add it to yours?</p>` +
          `<button class="action-flash200">` +
          `<span id="yes-action-flash200">Yes!</span>` +
          `<span id="no-action-flash200">No!</span>` +
          `</button>` +
          '</aside>';

    const flashMessage = {
      persistent: true,
      payload: standardContent,
    };
    return flashMessage;
  }
...
export default (app) => {
  return {
    initializeCapability(req, res, next) {
      ...
      if (!req.session.state) app.locals.capability.seedState(req.session, "anonymous");
    },
